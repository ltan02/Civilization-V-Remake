...
def drawUnits(canvas, data):
    for eachPlayer in data.allPlayers:
        if eachPlayer == "usa":
...
def singlePlayerMousePressed(event, data):
    player = data.singlePlayer
    for rows in range(len(data.boardPixel)):
        for cols in range(len(data.boardPixel[rows])):
...
def singlePlayerKeyPressed(event, data):
    ...
    if event.keysym == "space":
        ...
        if player == "usa":
            ...
            for eachUnit in data.frenchUnits and eachUnit[0].selected:
                if eachUnit[0] == "settler":
                    eachUnit[0].createCity(data)
                    break
        else:
            print("create city")
            for eachUnit in data.germanUnits and eachUnit[0].selected:
                if eachUnit[0] == "settler":
                    eachUnit[0].createCity(data)
                    break
...
def singlePlayerTimerFired(data):
    if data.gameStarted:
        for eachPlayer in data.allPlayers:
...
def singlePlayerRedrawAll(canvas, data):
